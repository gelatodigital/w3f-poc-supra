...
contract DIASignedOracleMultiple is Ownable {
    ...
    function setValues(Message[] memory vs) public {
        for (uint i = 0; i < vs.length; i++) {
            this.setValue(vs[i]);
        }
    }

    function setValue(Message memory value) external {
        require(getSigner(value) == signer, "SIGNER_DONT_MATCH");
        values[value.Symbol] = value;
        emit OracleUpdate(value.Symbol, value.Price, value.Time);
    }
    ...
    function getSigner(Message memory message) public view returns (address) {
